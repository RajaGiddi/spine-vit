...
def save(fig, stem):
    for _ in range(4):
        ...
        fig.set_size_inches(width + err, height)
...
def load_run(experiments_dir, prefix, seed):
    run_dir = os.path.join(experiments_dir, f"{prefix}_s{seed}")
    p = os.path.join(run_dir, "test_results.json")
    test_results = json.load(open(p))
    ...
    return metrics.get("kappa"), att.get("worst_level_accuracy"), p


def figure3(experiments_dir, log):
    stats_by_tok, sources = [], []
    for label, prefix in TOKENIZERS:
        kappas, worst_levels = [], []
        for s in TOK_SEEDS:
            kappa, worst_level, p = load_run(experiments_dir, prefix, s)
            kappas.append(kappa)
            worst_levels.append(worst_level)
            sources.append(p)
        stats_by_tok.append((label, np.mean(kappas), np.std(kappas), np.mean(worst_levels), np.std(worst_levels)))

    fig, ax = plt.subplots(figsize=(FIGW, 2.1))
    count = len(stats_by_tok)
    width = 0.19
    centres = np.array([0.0, 1.12])
    offs = (np.arange(count) - (count - 1) / 2) * width

    for i, (label, km, kappas, wm, worst_levels) in enumerate(stats_by_tok):
        x = centres + offs[i]
        first = i == 0
        ax.bar(x, [km, wm], width=width * 0.92, yerr=[kappas, worst_levels], color=PALETTE[i],
    ...
    for label, km, kappas, wm, worst_levels in stats_by_tok:
        log.append(f"    {label:<16} kappa {km:.3f}+/-{kappas:.3f}   worst_lvl {wm:.3f}+/-{worst_levels:.3f}")
    log += [f"    {p}" for p in sources]
...
def figure4(experiments_dir, log):
    ...
    for tag, prefix in (("sag1", SAG1), ("sag5", SAG5), ("axial", AXIAL)):
        for s in PLANE_SEEDS:
            _, worst_level, p = load_run(experiments_dir, prefix, s)
            wl[(tag, s)] = worst_level
            sources.append(p)
    ...
    pvals = [stats.ttest_rel(np.array([wl[(b, s)] for s in PLANE_SEEDS]),
                             np.array([wl[(attribution, s)] for s in PLANE_SEEDS])).pvalue
             for attribution, b in (("sag1", "sag5"), ("sag5", "axial"))]
    ...
    for i, (_, run_dir) in enumerate(cols):
        ax.plot([x_values[i] - 0.16, x_values[i] + 0.16], [run_dir.mean()] * 2, color=INK, lw=2.0,
                solid_capstyle="butt", zorder=4)

    top = max(budget.max(), acquis.max())
    for i, ((label, run_dir), pv) in enumerate(zip(cols, pvals)):
        ax.text(x_values[i], top + 0.028, f"{run_dir.mean():+.3f}, p={pv:.3f}", fontsize=6.5,
                ha="center", va="bottom", color=INK)
    j45 = PLANE_SEEDS.index(45)
    ax.text(x_values[0] + jit[j45] - 0.035, budget[j45], "seed 45", fontsize=6, color="0.45",
            ha="right", va="center")
    ...
    for j, s in enumerate(PLANE_SEEDS):
        ...
        parts = []
        for nm, run_dir in (("budget", budget), ("acquisition", acquis)):
            kept_deltas = run_dir[keep]
            t = stats.ttest_1samp(kept_deltas, 0.0)
            ci = stats.t.ppf(0.975, len(kept_deltas) - 1) * kept_deltas.std(ddof=1) / np.sqrt(len(kept_deltas))
            parts.append(f"{nm} {kept_deltas.mean():+.3f} p={t.pvalue:.3f} [{kept_deltas.mean()-ci:+.3f}, {kept_deltas.mean()+ci:+.3f}]")
...
def smooth(y, worst_level=SMOOTH):
    y = np.asarray(y, dtype=float)
    return np.array([y[max(0, i - worst_level // 2):i + worst_level // 2 + 1].mean() for i in range(len(y))])


def figure_curves(experiments_dir, log, seed=None):
    survey = {}
    for run_dir in sorted(glob.glob(os.path.join(experiments_dir, f"{CURVE_PREFIX}_s*"))):
        ...
        metrics = test_results.get("metrics_full") or test_results.get("metrics") or {}
        survey[int(config["seed"])] = dict(kappa=metrics["kappa"], best_epoch=test_results["best_epoch"],
                                       n_epochs=len(history["train_loss"]), cap=config["epochs"],
                                       hist=history, dir=run_dir)
    mean_k = float(np.mean([v["kappa"] for v in survey.values()]))

    if seed is None:
        epoch_counts = {v["n_epochs"] for v in survey.values()}

        def how_many_runs(epoch_count):
            total = 0
            for value in survey.values():
                if value["n_epochs"] == epoch_count:
                    total = total + 1
            return total

        modal_n = max(epoch_counts, key=how_many_runs)
        cands = [s for s, v in survey.items() if v["n_epochs"] == modal_n] or list(survey)

        def distance_from_mean_kappa(candidate_seed):
            return abs(survey[candidate_seed]["kappa"] - mean_k)

        seed = min(cands, key=distance_from_mean_kappa)
    r = survey[seed]
    history, best, count = r["hist"], r["best_epoch"], r["n_epochs"]
    early = count < r["cap"]
    ep = np.arange(1, count + 1)
    tr_l, va_l = np.array(history["train_loss"], dtype=float), np.array(history["val_loss"], dtype=float)
    ...
    for y, c, ls, lab in ((tr_l, ACCENT, "-", "Train loss"), (va_l, "#E69F00", (0, (4, 2)), "Val loss")):
        ...
        ax.plot(ep, smooth(y), color=c, ls=ls, lw=0.9, zorder=3, label=lab)
    ...
    log.append(f"    early stopping {'triggered' if early else 'did NOT trigger'}: "
               f"{count} of {r['cap']} epochs ran; final epoch {count-1} "
...
def main():
    ...
    parser.add_argument("--curve_seed", type=int, default=None, help="override the training-curve seed")
