"""Aggregate the v2 Task 2 fusion sweep: 4-way ablation + axial box-size dose-response.

Reports canal-stenosis metrics on the FULL test set and the AXIAL-AVAILABLE subset,
mean +/- std over seeds, and applies the pre-registered interpretation bands
(Delta>=0.09 claim / 0.06-0.09 trend / <0.05 n.s.) against the sagittal-only control.

Run:  ./.venv/bin/python scripts/aggregate_fusion.py --experiments_dir outputs_modal
"""
...
# median physical extent (mm) of each 224-space box px on axial (from box_mm_readout)
...
def main():
    ...
    # ---- Table 1: 4-way ablation at axial_box=32 ----
    ...
    # ---- Table 2: axial box dose-response (fusion-B attn) ----
